...
func main() {
	...
	remoteAPIURL := []string{
		"https://vulcanmaterials-us-e.azurewebsites.net/construction-materials/safety-data-sheets",
	}
	...
	// Log the number of unique PDF links found
	log.Printf("Found %d unique PDF links", len(finalPDFList))
	remoteDomain := "https://vulcanmaterials-us-e.azurewebsites.net" // Define base domain for relative links
...
func urlToFilename(rawURL string) string {
	...
	lowercaseURL = strings.SplitN(lowercaseURL, "?", 2)[0] // Remove query parameters if present
...
// extractPDFUrls parses an HTML string and returns all .pdf links from <a href="..."> tags
func extractPDFUrls(htmlContent string) []string {
	doc, err := html.Parse(strings.NewReader(htmlContent)) // Parse HTML input
	if err != nil {
		return nil // Return nil if the HTML is invalid
	}

	var pdfLinks []string // Slice to hold PDF links

	var traverse func(*html.Node) // Declare recursive traversal function
	traverse = func(n *html.Node) {
		if n.Type == html.ElementNode && n.Data == "a" { // Check if node is an <a> tag
			for _, attr := range n.Attr { // Loop through <a> tag attributes
				if attr.Key == "href" && strings.Contains(strings.ToLower(attr.Val), ".pdf") {
					// Check if href contains ".pdf" (case-insensitive), even with query params
	...
	traverse(doc)   // Begin recursive traversal at root
	return pdfLinks // Return collected PDF URLs
}
